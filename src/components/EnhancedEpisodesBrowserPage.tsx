...
export function EnhancedEpisodesBrowserPage({ 
  ...
  // Add this as a temporary component to test the TMDB API directly

  import React, { useState } from 'react';

  export function TMDBAPITester() {
    ...
    const testTMDBAPI = async () => {
      ...
      // Test with Breaking Bad (popular show that should have streaming data)
      const BREAKING_BAD_IMDB = 'tt0903747';
      const BREAKING_BAD_TMDB_ID = 1396;

      try {
        console.log('[TMDB TEST] Starting direct API tests...');

        // Test 1: Get TV series basic info
        const basicUrl = `https://api.themoviedb.org/3/tv/${BREAKING_BAD_TMDB_ID}?api_key=${API_KEY}`;
        console.log('[TMDB TEST] Basic URL:', basicUrl.replace(API_KEY, 'HIDDEN'));
        
        const basicResponse = await fetch(basicUrl);
        const basicData = await basicResponse.json();
        
        // Test 2: Get watch providers separately
        const watchProvidersUrl = `https://api.themoviedb.org/3/tv/${BREAKING_BAD_TMDB_ID}/watch/providers?api_key=${API_KEY}`;
        console.log('[TMDB TEST] Watch providers URL:', watchProvidersUrl.replace(API_KEY, 'HIDDEN'));
        
        const watchResponse = await fetch(watchProvidersUrl);
        const watchData = await watchResponse.json();
        
        // Test 3: Get with append_to_response
        const appendUrl = `https://api.themoviedb.org/3/tv/${BREAKING_BAD_TMDB_ID}?api_key=${API_KEY}&append_to_response=watch/providers`;
        console.log('[TMDB TEST] Append URL:', appendUrl.replace(API_KEY, 'HIDDEN'));
        ...
        const results = {
          basicData: {
            name: basicData.name,
            id: basicData.id,
            keys: Object.keys(basicData),
            hasWatchProviders: 'watch/providers' in basicData
          },
          watchProvidersData: {
            status: watchResponse.status,
            data: watchData,
            regions: watchData.results ? Object.keys(watchData.results) : []
          },
          appendData: {
            name: appendData.name,
            keys: Object.keys(appendData),
            hasWatchProviders: 'watch/providers' in appendData,
            watchProviders: appendData['watch/providers']
          }
        };

        console.log('[TMDB TEST] Results:', results);
    ...
    return (
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <h3 className="font-bold text-blue-800 mb-4">🧪 TMDB API Direct Test</h3>
